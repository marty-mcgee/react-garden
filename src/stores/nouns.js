...
const nounStore = (nounType = 'noun') => create({
  ...
  all: [], // working all
  one: {}, // the current working noun, aka 'this one noun'
...
const nounActions = (nounType = 'noun') => ({
  ...
  removeAll: function () {
    localStorage.removeItem('threed_nounHistory')
    ...
    console.debug('%cremoveAll [all]', ccm2, true)
  },

  // add a new current 'this' noun
  addNew: function () {

    console.debug('%caddNew [all] (before)', ccm1, nounStore().get('all'))

    // create a new one
    if (Object.keys(nounStore().get('one')).length === 0) {
      try {
        nounStore().update('one', noun())
      } catch (err) {
        console.error('ADD NEW noun() err', err, noun)
      }
    }
    // save + update old one
    else {
      ...
      console.debug('%caddNew [all] (during)', ccm1, nounStore().get('all'))
    ...
    console.debug('%caddNew {noun}', ccm1, nounStore().get('one'))
    ...
    console.debug('%caddNew [all] (after)', ccm1, nounStore().get('all'))
    ...
    console.debug('%caddNew {noun} final', ccm1, nounStore().get('one'))
  },
        ...
        if (data.all?.edges?.length) {
          ...
          const all = payload.map((node) => {
            try {
              ...
              console.error('LOAD FROM DB noun() err', err, noun)
            }
          })
...
// const sceneStore = nounStore('scene')
